...
7. Saves each OSINT page as raw HTML only.
8. Saves email attachments into the same analyst-facing folder as the OSINT HTML files.
9. Generates one ASCII-sanitized AI prompt file.
10. Writes manifests and failure reports without stopping on individual errors.
11. Cleans up the temp MSG copy after execution.
    ...
    foreach ($item in $osintItems) {
        ...
        try {
            ...
            try {
                ...
                $result.Error = $msg
                $manifest += New-ManifestEntry -Title $item.Title -Url $item.URL -HtmlFile '' -Method 'Failed' -Status 'Failed' -Blocked $false -BlockReason ''
                continue
            }
        }
        ...
        try {
            ...
            $manifest += New-ManifestEntry -Title $item.Title -Url $item.URL -HtmlFile $htmlPath -Method $methodUsed -Status 'Success' -Blocked $result.Blocked -BlockReason $result.BlockReason
