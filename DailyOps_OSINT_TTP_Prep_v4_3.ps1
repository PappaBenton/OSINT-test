...
<#
.SYNOPSIS
Hardened Daily Ops OSINT extraction prep script with temp MSG open workaround,
MSRC-specific CVE handling, and challenge-aware tagging.
...
What it does:
1. Locates the first .msg file in the working directory.
2. Copies the .msg to a uniquely named temp file before opening it in Outlook.
3. Trims the email body at the FYSA marker.
4. Extracts OSINT titles and URLs from the message body.
5. Saves URL lists and logs for troubleshooting.
6. Fetches each URL using browser-like headers, per-site session cookies, and an HttpClient fallback.
7. Detects MSRC CVE URLs and creates an enriched HTML wrapper from Microsoft Security Update API metadata.
8. Detects likely challenge/wait/interstitial pages and tags them in the manifest and logs.
9. Saves each OSINT page as HTML only.
10. Saves email attachments into the same analyst-facing folder as the OSINT HTML files.
11. Generates one ASCII-sanitized AI prompt file.
12. Writes manifests and failure reports without stopping on individual errors.
13. Cleans up the temp MSG copy after execution.
...
function Get-MsrcCveFromUrl {
    ...
    if ($m.Success) {
        return $m.Groups['Cve'].Value.ToUpperInvariant()
    }
    return $null
}
...
function Build-MsrcHtmlWrapper {
    ...
    if ($ApiData -and $ApiData.RawJson) {
        $raw = $ApiData.RawJson
        if ($raw.Length -gt 25000) {
            $raw = $raw.Substring(0,25000)
        }
...
try {
    ...
    $osintItems = foreach ($m in $matches) {
        ...
        if ($url.Length -gt $maxUrlLengthForPrompt) {
            $url = $url.Substring(0, $maxUrlLengthForPrompt)
        }
    ...
    foreach ($item in $osintItems) {
        ...
        try {
            ...
            if ($isMsrcCve) {
                Write-Log -Level INFO -Message ("Detected MSRC CVE URL: {0}" -f $msrcCve)
                ...
                if ($apiData -ne $null) {
                    ...
                    $notes = 'MSRC API retrieval did not return data; wrapper includes fetched page preview only.'
                    Write-Log -Level WARN -Message ("No MSRC API data returned for {0}" -f $msrcCve)
                }

                $wrappedHtml = Build-MsrcHtmlWrapper -Cve $msrcCve -OriginalUrl $item.URL -PageHtml $html -ApiData $apiData
                Save-HtmlFile -Path $htmlPath -Html $wrappedHtml | Out-Null
            }
            else {
                Save-HtmlFile -Path $htmlPath -Html $html | Out-Null
            }
...
"@
    ...
    if ($tempMsgPath -and (Test-Path -LiteralPath $tempMsgPath)) {
        try {
            Remove-Item -LiteralPath $tempMsgPath -Force -ErrorAction SilentlyContinue
        }
        catch {}
    }
}
