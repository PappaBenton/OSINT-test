...
try {
    ...
    Write-Log -Level INFO -Message 'Saved OSINT title/link outputs into Ready for TTP Extractions.'
    ...
    foreach ($item in $osintItems) {
        ...
        try {
            ...
            try {
                ...
                $result.Notes = 'Manual review required'
    ...
    if (($results | Measure-Object).Count -gt 0) {
        $results |
            Select-Object Status, 'Article Title', URL, Notes |
            Export-Csv -Path $urlStatusCsv -NoTypeInformation -Encoding UTF8
        Write-Log -Level INFO -Message 'Generated unified scrape URL status CSV in Ready for TTP Extractions.'
    }
    ...
    $failureNoteText = "Successful scrapes: $successCount; Failed scrapes: $failCount"
