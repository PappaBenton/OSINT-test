...
function Read-YesNoPrompt {
    ...
    do {
        ...
        if ($response -match '^(?i)y(?:es)?$') { return 'Y' }
        if ($response -match '^(?i)n(?:o)?$') { return 'N' }
...
try {
    ...
    $excludedCount = @($osintItems | Where-Object { Test-BlockedDomain -Url $_.URL }).Count
    if ($excludedCount -gt 0) {
        Write-Log -Level WARN -Message ("Excluded {0} blocked-domain URL(s) from all outputs." -f $excludedCount)
    }

    $osintItems = @($osintItems | Where-Object { -not (Test-BlockedDomain -Url $_.URL) } | Sort-Object Order)

    if (($osintItems | Measure-Object).Count -gt 0) {
        $reordered = @()
        $newOrder = 1
        foreach ($item in $osintItems) {
            $reordered += [PSCustomObject]@{
                Order = $newOrder
                Source = $item.Source
                Title = $item.Title
                URL = $item.URL
        ...
        $osintItems = $reordered
    }

    if (($osintItems | Measure-Object).Count -eq 0) {
        Write-Log -Level WARN -Message 'No eligible OSINT URLs remained after blocked-domain exclusions.'
    }
    ...
    Write-Log -Level INFO -Message 'Saved ordered OSINT title/link outputs in original email order after exclusions.'
    ...
    if ($manualReviewItems.Count -gt 0) {
        Write-Host ''
        $answer = Read-YesNoPrompt -Prompt ("Open the {0} manual review URL(s) now? (Y/N)" -f $manualReviewItems.Count)
        if ($answer -eq 'Y') {
